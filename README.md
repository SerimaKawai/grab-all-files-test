...
| **動画（直リンク）** | v4.5.x で追加。`<a href="x.mp4">` / `<video src>` / `<source src>` / `data-mp4` 等の直リンク（mp4/mov/mkv/webm/avi/wmv/flv… 20 種、いずれも extRe ゲート）。2026-06-13 に `<video src>`/`<source src>` も検出対象へ転換。音声・HLS/DASH・拡張子なし・blob は extRe ゲート脱落＋`avExtRe` で恒久除外 | 13 |
...
| 直リンク動画 | `pages/13-media-and-svg.html` | `<a href>` / `<video src>` / `<source src>` 動画は検出（extRe ゲート、期待17件）、音声・HLS・blob は**非検出** |
