...
- **対応拡張子**: `lib/file-utils.js` の `knownExtRe` を展開した **214 拡張子**（`docx?` 等の短縮表記を実拡張子に展開した実数。PDF、Office、画像、カメラ RAW、動画（直リンク）、CAD/3D、科学計算、証明書、フォント、コード設定など）
- **45+ 検出手法**: `popup.js` / `page-interceptor.js` の検出ロジックを網羅（v4.5.1 時点）
...
| プラットフォーム特化 | Gmail (`download_url` / `disp=safe→attd`), Yahoo!メール (`li[data-cy]` / thumbnail), Outlook (`role="option"` / blob:), **Salesforce Experience Cloud**（`.filerow` + `069` ContentDocument / `068` サムネ versionId → `sfc/servlet.shepherd/` URL 合成） | 11 |
| マジックバイト | %PDF, PK, .PNG, JPEG/GIF/WebP/WAV/AVI/MP4/Ogg/FLAC/MP3/RAR/7z/gz/BMP/BZ2/XZ/EXE/ASF | 14 |
| CSS | inline style, computed `backgroundImage` / `borderImageSource` / `listStyleImage`, `image-set()` のみ（**`@font-face` / `cursor` / `::before content:url()` は非検出**）。`::before content` の **種別ラベル**は `gafPseudoText` でファイル名へ折り込み | 02, 15 |
| **動画・音声（直リンク）** | v4.5.x で動画、v4.5.3 で音声を追加。`<a href="x.mp4">`/`<a href="x.mp3">` / `<video src>`/`<audio src>` / `<source src>` / `data-mp4`/`data-mp3` 等の直リンク（動画20種＋音声25種、いずれも extRe ゲート）。HLS/DASH・拡張子なし・blob は extRe ゲート脱落＋`avExtRe` で恒久除外 | 13 |
| **文脈ファイル名生成** | 過去問 DB の汎用「問題PDF/解答PDF」→ `学校_年度_科目_種別.pdf` 合成。hex `/Title` デコード、列ヘッダ科目、URL トークン補強（URL 単独では発火しない） | 16 |
| **アクティブ取得 / POST** | Active Capture Lv2（hrefless トリガー → クリックで傍受）、Tokio Marine SPS（`download_btn`/`thumbnail_image` → POST フォーム） | 17 |
| **クローラのナビ抽出** | `javascript:` / `onclick` のパス形リテラル（`jsUrlLiteralRe`）、`.php`/`.asp` ナビ（`crawlFileExtRe` 除外で enqueue）、`icon-pdf` 拡張子なしリンク | 18 |
| 誤検出フィルタ | SourceForge `/files`/`/reviews`/`/admin`, naturalearthdata `/download/<slug>/`, Liferay AJAX, **セッション破壊リンク**（`isSessionMutatingLink`：logout/signout はクロールしない） | 09 |
...
│   ├── 09-misdetection-traps.html   # 除外されるべきナビ URL + セッション破壊リンク
│   ├── 10-all-extensions.html       # 全 214 拡張子の網羅リンク
│   ├── 11-platform-mocks.html       # Gmail / Yahoo / Outlook / Salesforce Experience Cloud
│   ├── 12-pdfjs-viewer.html         # PDF.js viewer / PDF 閲覧 URL
│   ├── 13-media-and-svg.html        # 直リンク動画（v4.5.x）/ 動画・音声要素 / SVG / 短縮 data-*
│   ├── 14-magic-bytes.html          # 拡張子ミスマッチ → header 判定
│   ├── 15-css-scanning.html         # CSS から URL 検出 + ::before 種別 (gafPseudoText)
│   ├── 16-contextual-naming.html    # 文脈ファイル名生成（hex Title / 列ヘッダ / URL トークン）
│   ├── 17-active-capture.html       # Active Capture Lv2 / Tokio Marine SPS POST
│   ├── 18-crawl-nav.html            # クローラのナビ抽出（javascript:/onclick/icon-pdf/.php）
│   ├── crawl/                       # 18 から辿られる子ページ
│   └── deep/{level-1,2,3}.html      # クロール深度テスト
└── files/                           # サンプル実体ファイル (約 220 件)
```
...
| 確認したいこと | 開くページ | 期待 |
|---|---|---|
| 全拡張子の認識 | `pages/10-all-extensions.html` | 214 件すべて検出 |
...
| プラットフォーム | `pages/11-platform-mocks.html` | download_url / 069・068 → shepherd URL 合成 |
| 直リンク動画・音声 | `pages/13-media-and-svg.html` | `<a href>` / `<video src>`/`<audio src>` / `<source src>` の動画＋音声は検出（extRe ゲート、期待22件）、HLS/DASH・blob は**非検出** |
| マジックバイト | `pages/14-magic-bytes.html` | mystery.bin / archive.dat の種類列が pdf / zip |
| CSS スキャン | `pages/15-css-scanning.html` | background / image-set / border-image を抽出（@font-face・cursor・::before content url は**非検出**）。::before の種別→ファイル名 |
| 文脈ファイル名生成 | `pages/16-contextual-naming.html` | 「タイトルでリネーム」ON で `学校_年度_科目_種別.pdf`、hex `/Title` は `国語` に復元 |
| アクティブ取得 / SPS | `pages/17-active-capture.html` | hrefless トリガーが候補化（クリックで傍受）、SPS は POST 経路 |
| クローラのナビ抽出 | `pages/18-crawl-nav.html` | 深度1で javascript:/onclick/icon-pdf/.php 経由の子ページ PDF に到達 |
| 誤検出フィルタ | `pages/09-misdetection-traps.html` | SourceForge / logout 等が結果に**現れない**（logout はクロールもしない） |
