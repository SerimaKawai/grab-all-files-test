...
| **動画（直リンク）** | v4.5.x で追加。`<a href="x.mp4">` / 直リンク URL のみ（mp4/mov/mkv/webm/avi/wmv/flv… 20 種）。音声・HLS/DASH・`<video src>`/`<source>`・blob は `avExtRe` で恒久除外 | 13 |
...
| 直リンク動画 | `pages/13-media-and-svg.html` | `<a href>` 動画は検出、音声・HLS・`<video src>`・blob は**非検出** |
